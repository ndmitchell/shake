...
function runReport() {
    ...
    try {
        switch (report.mode) {
            case "summary":
                var mp = {};
                for (var i_2 = 0; i_2 < profile.length; i_2++)
                    mp[i_2] = [i_2];
...
function reportSummary(filter) {
    ...
    for (var k in filter) {
        for (var _i = 0, _a = filter[k]; _i < _a.length; _i++) {
            var i = _a[_i];
            ...
            if (!traces)
                continue;
            for (var _b = 0, traces_2 = traces; _b < traces_2.length; _b++) {
                var t_5 = traces_2[_b];
