...
module Development.Shake.Internal.Core.Action(
    ...
    getVerbosity, putWhen, putDebug, putInfo, putError, withVerbosity, quietly,
    orderOnlyAction,
    newCacheIO,
    unsafeExtraThread,
    parallel,
...
-- | Write an unimportant message to the output, only shown when 'shakeVerbosity' is higher than normal ('Debug' or above).
--   The output will not be interleaved with any other Shake messages (other than those generated by system commands).
putDebug :: String -> Action ()
putDebug = putWhen Debug

-- | Write a normal priority message to the output, only supressed when 'shakeVerbosity' is 'Error' or 'Silent'.
--   The output will not be interleaved with any other Shake messages (other than those generated by system commands).
