...
shakeOptsEx =
    ...
    ,yes $ Option "B" ["always-make"] (noArg $ \s -> s{shakeAssume=AssumeDirty}) "Unconditionally make all targets."
    ...
    ,yes $ Option ""  ["old-all"] (noArg $ \s -> s{shakeAssume=AssumeClean}) "Don't remake any files."
    ,yes $ Option ""  ["assume-skip"] (noArg $ \s -> s{shakeAssume=AssumeSkip}) "Don't remake any files this run."
    ...
    ,yes $ Option ""  ["touch"] (noArg $ \s -> s{shakeAssume=AssumeClean}) "Assume targets are clean."
