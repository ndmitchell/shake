...
\section{Delaying Dependency Specification}
\label{sec:theory}

Most \make{}-like build systems start by constructing a graph from the dependency information, then traverse the graph, running the rules to build the required results. However, any approach based on a static dependency graph cannot permit additional dependencies to be specified while running the rules. As we saw in \S\ref{sec:introduction}, many examples \textit{require} additional dependencies while running rules. The solution is simple -- we reject the idea of using a static dependency graph. In practice, we have found that eliminating an explicit dependency graph makes the the build system simpler!

In this section we describe the theory that underpins of the use of non-recursive \make{} as described by \citet{miller:recursive_make}, then show how to how the specification of dependencies. We then describe what it means for a build system to be correct, and how to support minimal rebuilds. In \S\ref{sec:user_view} and \S\ref{sec:developer_view} we show how to turn these ideas into a usable tool.

\subsection{Moving Dependencies}

If we assume that Key/Value is enough, and ignore any complexities of IO or parallellism, we can define:

\begin{code}
make :: Set Rule -> Key -> Value
\end{code}

And then we can define a Rule in Make as:
...
Note here that we can add additional dependencies as we are running. The second system is strictly more powerful.

\subsection{Correctness}

A Make build system is correct if you can build a finite graph that produces the target, assuming all the action functions are total. Alternatively:

\begin{code}
make rules target = run (rules ! target)
    where run x = action x (map (make rules) (depends x))
\end{code}

If this function terminates then |make| is well specified.

A Shake build system has a much less simple correctness property. However, it is simple to give a build system that never worries about rebuilding:

\begin{code}
make rules target = run (action (rules ! target))
    where  run (Result val) = val
           run (Depends dep act) = run (act (make rules dep))
\end{code}

A Shake build system is correct iff the |shake| function is total, assuming |!| throws an exception if it can't find a rule.
...
data  Rule = Rule
      {  creates  :: Key
      ,  depends  :: List_alpha Key
      ,  action   :: List_alpha Value -> Value
      }

make :: Set Rule -> Key -> Value
\end{code}

The |make| function takes a (possibly infinite) set of rules and the target |Key| to build, and returns the |Value| associated with that |Key|. A rule can be modelled as the |Key| it |creates|, the |Key|s it |depends| on, and the |action| that takes the depended upon |Value|s and produces the result |Value|. We use \h{type}|List_alpha bullet| to denote a list which must have length |alpha|, requiring that the |depends| list and the list passed to |action| are equal in length.
...
We say a |Rule| is associated with a |Key| if it has that |Key| as its |creates| field. Given a |Rule| |r| we can define \ignore|dependsS r| as all the |Key|s this rule requires to run, including the dependencies of its dependencies. We can compute \ignore|dependsS r| as the union of \ignore|depends r| and \ignore|dependsS| on all the rules associated to \ignore|depends r|.
...
\item \textit{Finite} - for all rules, \ignore|dependsS r| must be finite.
\item \textit{Acyclic} - for all rules, \ignore|dependsS r| must not contain \ignore|creates r|.
\end{enumerate}
...
\item To ensure that the recursion in |fromKey| terminates, we require that no |Rule| depends on its \h{.theory2}|creates| value, either directly or through a sequence of other rules.
...
If we restrict ourselves to |Key|s which are files, and |Value|s which are modification times (as \make{} does), then we can write a function |modified| which returns |Just| the modification time, or |Nothing| to indicate the file does not exist (and thus has never been built). An |Rule|s |action| can be skipped if the |creates| file exists, and all the |depends| times are older. Before running a rules action, all its dependencies must already have been created, and thus history must always return |Just|. We can check if a Rule is valid with respect to the history with:
