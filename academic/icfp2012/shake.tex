...
A build tool, such as \make{} \cite{make,recursive_make}, takes a set of input files, plus some build rules, and produces some output files. Using \make{}, a build rule can be written as:
...
\noindent This rule says that the file \file{result.tar} depends on the inputs \file{file1} and \file{file2} (first line), and provides a command to build \file{result.tar} (second line). Whenever \file{file1} or \file{file2} change, the command will be run, and \file{result.tar} will be regenerated.

But imagine we want to build \file{result.tar} from the list of files stored in \file{list.txt}. The dependencies of \file{result.tar} are no longer static, but depend on the contents of \file{list.txt}. Unfortunately, \make{} offers no easy way to express this pattern (there are workarounds, but none are pleasant or effective). Using the build tool we develop in this paper, we can write:
...
In this section we describe the theory that underpins of non-recursive \make{} \cite{non-recursive_make}, then the underlying theory of our build system, named Shake. We then describe how to support minimal rebuilds in both theories. In \S\ref{sec:user_view} and \S\ref{sec:developer_view} we show how to implement these ideas in a usable tool.
