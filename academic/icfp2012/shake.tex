...
\item [NFData] We require |NFData| to ensure that values are fully evaluated when they are computed, ensuring that errors occur in a timely manner (\S\ref{sec:shake_state}).
\end{description}
...
To turn Shake into a practical system, we make a number of changes from the natural implementation, designed to improve the situation when errors occur.

\paragraph{Tracking the stack} We maintain a stack while executing rules, listing the keys that cause a rule to be executed. Whenever an error occurs, either when running a rule or trying to find a rule, we print the stack. The stack is particularly useful when there is no rule for a key, but the fact that key is required is itself suspicious. Before starting a rule we given an error if the key is already on the stack, which would indicate a key depends on itself. This check allows us to give a clear error message where otherwise there would have been an infinite loop.
...
In the implementation of Shake there are several goals:


Efficient, in particular we want to avoid fork-bombing when we have nothing to do.

Parallel - kick off tasks immediately.
