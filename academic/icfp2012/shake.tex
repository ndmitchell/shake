...
The Shake database records the dependencies of each item, allowing a full dependency graph to be produced. However, for any project of moderate size, the result is rarely human readable -- although through judicious filtering it is possible to produce something useful (see Figure \ref{fig:analysis} for an example). Instead, we have found that writing passes to answer specific questions

Instead

Show me a full dependency graph. While this question is asked, for larger projects it rarely produces something human readable -- although through judicious filtering we have produced useful graphs. A far more interesting problem is to state a list of expected invariants on the build system (e.g. modules outside |Development.Shake| should not be importing things from inside |Development.Shake| directly).
