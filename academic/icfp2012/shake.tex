...
\item [Hashable] We require |Hashable| to lookup entries quickly \cite{unordered-containers}.
...
These operations mirror their standard counterparts, but automatically include a |need| call. We also define a number of additional functions for working with files. The functions |readFileLines| and |writeFileLines| deal with files which should be treated as a list of lines. The function |writeFileChanged| writes a file, but only if the contents have changed, avoiding rebuilding by making use of unchanging files (\S\ref{sec:unchanging_files}).
...
The key algorithm in Shake is implemented in 130 lines of Haskell. It takes a list of keys, and produces either a list of values, or raises an error if one of the results raises an error. This algorithm is parameterised by a way to check a rule is valid, and a way to execute a rule. At its heart, each |Key| is in one of seven states, and this algorithm moves keys between those states. When implementing this algorithm, there are two primary concerns. Firstly, the algorithm must be correct and secondly it must be efficient.

On top of this algorithm, it is relatively easy to implement the core of Shake. We execute the top-level |action|s, and whenever we call |apply|, we look up the state in the database, and use the transition diagram in Figure \ref{fig:state} to make sure everything is |Built| before continuing.

\subsubsection{Correctness}

Must never run a rule twice. Must not start building something unless necessary, including checking for changing files. Must not throw an error that the build system did not cause. These two requirements are somewhat in conflict, to take advantage of unchanging files we must build a keys old dependencies before running the action. However, that might raise an error -- yet if we had rerun the rule from scratch, the build system may never have required that file. Therefore we need to note this state.

\begin{figure}
\begin{center}
\includegraphics[scale=0.5]{states.eps}
\end{center}
\caption{Result states and transitions.}
\label{fig:states}
\end{figure}

The state transition is given in Figure \ref{fig:states}. A key starts as either loaded (we had a previous definition for it), or missing (it was previously unknown to the build system).  If the file was missing, and is required, we start building it, by running its action. Building terminates with either a result, or an error. If a file is loaded then when it is required we start checking it. If all the dependencies are ready, we check them and |validStored|, and transition to either |Building| (something is changed) or |Ready| (the previous result is valid). If one of the dependencies results in an error then if the key is directly required by |build| we switch to building, otherwise we switch to Dirty.
...
In order to obtain correctness, we need 7 states. Explain each state.
...
Maximise the parallelism up to a given limit, kick off things as soon as possible. A rebuild should be quick if nothing needs to change, ideally no locking. Once we start building something we always finish it.

Must be fast, by making doing-nothing cheap, and by making doing-something parallel. We optimisitcally lock etc. while remembering that often executing a rule is expensive.

We never move to the checking state unless we depend on someone who is building or checking, if we can check straight through we skip that.
...
While implementing a large build system we have learnt a number of lessons -- both about best practices for structuring build systems, and how the underlying core of Shake can be used to deal with the complexities of real software. In this section we share some of those lessons.
