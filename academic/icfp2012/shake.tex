...
While the timestamp approach works reasonably for \make{}, it can fail the system clock changes, or if a file is modified but has its value set to an older timestamp -- such as when extracting a backup which resets the timestamp. Instead of relying on the system time, we can instead store a counter in our database, and increment it every time we require a time value, thus guaranteeing that our time is correctly ordered.
...
data Stored alpha  =  NeverStored | NotStored | Stored alpha
...
class (
    ...
    externalValue :: key -> IO (Stored value)
    externalValue x = return NeverStored
...
defaultRule :: Rule key value => (key -> Make value) -> Rules ()
