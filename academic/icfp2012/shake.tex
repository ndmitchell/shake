...
In this section we describe the underlying basis of \make{}, then the underlying basis of our build system, named Shake. Finally, we describe how to support minimal rebuilds in Shake. In \S\ref{sec:user_view} we show how to implement these ideas in a usable tool.
...
We restrict our model to only building one target, while \make{} allows multiple targets (i.e. a list of |Key|s to build). However, we can encode multiple targets by creating a distinguished rule that depends on all the targets and returns all their values, and then make that rule the target.

\subsubsection{Correctness}

We say a |Rule| is associated with a |Key| if it has that |Key| as its |creates| field. Given a |Rule| |r| we can define |dependsS r|, the complete set of |Key|s this rule requires to run, including those rules required by rules it requires. We can compute |dependsS r| as the set that is the union of |depends r| and |dependsS| on all the rules associated to the keys.
...
\noindent Given these conditions, there exists a finite list of |action|s that when run in order produces the result.
...
The big difference from |make| is the introduction of dynamic dependencies. A rule can dynamically, based on the values of previous dependencies, add additional dependencies.

\subsubsection{Correctness}

A call to |shake| is correct if the |Value| can be built from the target |Key|. Unlike |make|, we cannot check this property before running any |action|s, since new dependencies can be added by running an |action|. Like |make|, we require that each |Key| is associated with exactly one |Rule|, we also classify rules:

At any point there is exactly one rule that can be reduced and must be!
