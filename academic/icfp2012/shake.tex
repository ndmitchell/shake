...
|Result_m| contains the |Key| the action |created|, the |result| |Value|, and the time when the result was |built|. We store all |Result_m| values between build runs, and skip rerunning a |Rule_m| if: 1) the |create| |Key| has a result available; 2) all the |depends| |Key|s have results available; and 3) the |built| field of the result associated with |create| is greater than or equal to all the |built| fields of the results associated with |depends|. Given a function named |ask| to lookup the associated result for a |Key|, which returns |Nothing| if the there is no associated result, we can write:
...
Compared to |skip_m|, we have made one change -- instead of using |depends r|, we use |depends self|. With this modification we are able to ensure minimal rebuilds using |Rule_s|. We still require the |validStored| check from the previous section.
...
We have only changed the final line of |skip_s| -- instead of checking against the |built| time of the dependencies, we check against the |changed| time. Since it is always the case that |changed <= built|, |skip_s| will now return |True| more often.
