...
When running the build system while nothing needs compiling, GHC takes 0.54 seconds and Shake takes 0.10 seconds. Of the 0.10 seconds required by Shake, 0.06 seconds is spent checking the GHC installation has not changed (running @ghc-pkg list@) -- if the GHC installation is assumed to be constant Shake requires only 0.04 seconds. Shake is faster because it reads in one file (the database) then quickly checks |validStored| on a small number of files. GHC must at least query the same file information, but also has to construct a dependency graph, aggregating information from many files. Of that 0.04 seconds, 0.03 is spent writing the database -- we suspect that effort spent improving the binary serialisation would pay off.

We also checked against a \make{} file generated by \prog{ghc -M}. Unlike the Shake and @ghc --make@ solutions, this didn't perform any checking of the environment via ghc packages, and if the structure of the package changed, the file would need to have been regenerated. The \make{} solution was roughly 0.09 seconds faster in all situations (0.10 seconds faster at single threaded, 0.08 faster at parallel rebuilding and 0.09 at the nothing rebuild). Of the 0.09, 0.06 is due to running @ghc-pkg@ and 0.03 to writing out the database. That suggests that the other overheads, namely of checking and dynamically deciding what to compile, are negligible. The identical amount of parallelisation suggests that Shake really is as good as \make{} at finding parallel opportunities. The Shake experience will be better for the end user, since they don't have to regenerate their make file explicitly.
...
Specifying additional dependencies while building is critical for many projects. As a result, a number of techniques have been developed to specify additional dependencies in \make{}:

\begin{itemize}
\item Many large projects introduce a two stage build, where one phase generates \make{} rules, and a second stage runs them. Often the first stage will be invoked manually, for example with @make depends@, and will be run less frequently than @make@.
\item Many \make{} implementations monitor their rule files (and any included rule files), and if they change, restart from the beginning. This feature allows dependencies to be regenerated by standard build rules.
\end{itemize}

Both of these workarounds can help, but neither is a complete solution. Both add overhead due to restarting \make{} and rechecking previously checked rules and reducing parallelism opportunities. Both require programs to generate \make{} rules for their dependencies (although most compilers support generating \make{} rules automatically).

%if 0
    Many compilers, such as \prog{gcc}, can generate \make{} rules. Therefore, authors introduce rules that generate fragments of \make{} rules, and if they have changed, \make{} restarts automatically.


Both of these workarounds can help, but neither is a complete solution. Both lose parallelism, and introduce additional overhead of restarting \make{}. Programs requiring restartable build systems are a pain, and involves lots of creating make rules (something made easier by the number of compilers that can already do this).
%endif
