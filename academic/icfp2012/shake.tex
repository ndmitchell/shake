...
The |Database| maps each |Key| to its |Status|. Draw a movement diagram.

\xymatrix{
 & \mathsf{Loaded}\ar@@/_1pc/[dr]^{Invalid}\ar@@/^1pc/[rr]^{Valid} & & \mathsf{Built} \\
\mathit{From disk}\ar@@{-->}[ur] & & \mathsf{Building}\ar@@/_1pc/[ur]^{Finished} \\
& Via \mathsf{need}\ar@@{-->}[ur]
}

none -> Loaded, loaded from disk
none -> Building, file that was needed (Info is set to none)
Loaded -> Building, was checked, found to be incorrect (Info is Just)
Loaded -> Built, was checked, found to be correct
Building -> Built, once it finishes


We enrich the |depends| field to be a list of keys. Things loaded from disk get |Loaded|, things which are built or have their loaded state validated are put into |Built| and things which are being built are in |Building|.
...
We journal step which moves something to Built as a result of changing it. That way
...
Haskell libraries: Blueprint, Coadjute, nemesis, cake, hake, hmk, zoom, openshake
