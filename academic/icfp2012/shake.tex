...
\end{code}
\h{.theory1}\begin{code}
modified :: Key -> Maybe Double
\end{code}
\h{.theory2}\begin{code}
find :: Set Rule -> Key -> Rule
\end{code}
\end{comment}
...
While the \make{} tool is heavily IO and file based, this is not an essential property of \make{}. To model dependency specification we use the type |Key| for things that can be created or are dependencies (i.e. file names) and the type |Value| for the values associated with a |Key| (i.e.  file contents). With these types, we can define our main |build| function as:

\begin{code}
build :: Set Rule -> Key -> Value
\end{code}

The |build| function takes a (possibly infinite) set of rules and the target |Key| to build, and returns the |Value| associated with that |Key|. We restrict our model to only building one target, while \make{} allows multiple targets (i.e. a list of |Key|s to build). However, we can encode multiple targets by creating a distinguished rule that depends on all the targets and returns all their values, and then make that rule the single target.

Using |build|, we can mode \make{} by defining the |Rule| type as:
...
A \make{} |Rule| can be modelled as the |Key| it |creates|, the |Key|s it |depends| on, and the |action| that takes the depended upon |Value|s and produces the result |Value|.

Using the same |build| function, we can model our enhanced dependency specification as:
...
A Shake |Rule| can be modelled as the |Key| it |creates|, and the |action| that creates the result. The |Action| either returns the |Result| |Value|, or requires a new dependency with |Depends| -- specifying the |Key| it depends on, plus a function that takes the |Value| of that |Key| and provides a new |Action|.

The big difference is the introduction of dynamic dependencies. A rule can dynamically, based on the values of previous dependencies, require additional dependencies. We can easily translate |Rule_m| to |Rule_s|, but the reverse is not possible -- |Rule_m| is strictly more powerful than |Rule_s|.
...
Starting at the |target| |Key|, we find the associated |Rule_m|, run |build_m| on its dependencies, and then run the |action|. A |Rule_m| build system is correct for a given target iff the |build_m| function is total. If we assume all |action|s are total, then |build_m| is total if you can build a finite dependency graph for the target. This property can be checked for a given target without running any actions.
...
Starting at the |target| |Key|, we find the |action| from the associated |Rule_s|, and run it. If we encounter a |Result| we are done, and if we encounter a |Depends| we run |build_m| on that dependency before continuing. As before, a |Rule_s| build system is correct for a given target iff the |build_s| function is total. However, unlike before, there is no obvious way to predict termination in advance without detailed information about the |action| functions.

\subsection{Minimal Rebuilds}
\label{sec:minimal_rebuilds}

One important property of build tools is ensuring minimal rebuilds, something we entirely ignored in the the previous section. In any execution of |build|, any |action| should be run at most once, a property that is easy to ensure with a simple cache. Additionally, if an |action| has the same inputs as the last time it was run, it should not be repeated. In this section we first explore how to avoid repeating actions whose inputs have not changed with |Rule_m|, enhancing the strategy followed by \make{}, then we apply the same ideas to |Rule_s|.

\subsubsection{Minimal Rebuilds with |Rule_m|}
\label{sec:cached_make}

To avoid repeating actions whose inputs have not changed, \make{} does not run any actions where the input files have older modification times than the result file. We can use a similar scheme:
...
skip :: Result -> [Result] -> Bool
skip self deps = all ((<= built self) . built) deps
\end{code}

Whenever an |action| is run, we create a |Result| containing the |Key| that action creates, the |Value| that is the result, and the time at which it was |built|. To store build times, we can keep use a variable that is incremented each run of the build system. With this scheme, if a file has already been built in a previous execution, and all its dependencies have been built, then we can use |skip| to test whether this action can be safely ignored, using the |value| stored in |Result|. Between build runs we must store all |Result| values, loading them in the next run.

Compared to \make{} this scheme is more general, allowing more than just files. However, what if the |Value| is also stored on disk, but has changed? We can simply check for 


which build-step it executes.


 by storing results from previous actions, remembering the |Key|, |Value| and time the result with built



Given a function |#| which returns Maybe 


\h{.theory1}\begin{code}
modified :: Key -> Maybe Value
\end{code}

If we restrict ourselves to |Key|s which are files, and |Value|s which are modification times (as \make{} does), then we can write a function |modified| which returns |Just| the modification time, or |Nothing| to indicate the file does not exist (and thus has never been built). A |Rule_m| can be skipped if:

\begin{code}
skip :: Rule_m -> Bool
skip 
    Nothing -> False
    Just t -> all ((< t) . fromJust . modified) (depends r)
\end{code}
 
An |Rule|s |action| can be skipped if the |creates| file exists, and all the |depends| times are older. Before running a rules action, all its dependencies must already have been created, and thus history must always return |Just|. We can check if a Rule is valid with respect to the history with:

\h{.theory1}\begin{code}
validHistory :: Rule -> Bool
validHistory r = case modified (creates r) of
    Nothing -> False
    Just t -> all ((< t) . fromJust . modified) (depends r)
\end{code}
...
To achieve minimal rebuilds with |Rule_m| we rely crucially on having the dependencies available without executing the action. However, it's simple to solve in |Rule_s|, we simply store the |depends| list in |Result|.
